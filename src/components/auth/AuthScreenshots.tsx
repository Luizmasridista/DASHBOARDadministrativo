          ...
          >
            <CarouselContent className="-ml-2 md:-ml-4">
              {softwareScreenshots.map((screenshot, index) => {
                ...
                return (
                  <CarouselItem key={index} className="pl-2 md:pl-4 basis-full lg:basis-4/5 xl:basis-3/4">
                    <motion.div
            ...
            <CarouselPrevious className="hidden lg:flex -left-12 bg-slate-800/80 border-slate-600/50 text-white hover:bg-slate-700/80" />
            <CarouselNext className="hidden lg:flex -right-12 bg-slate-800/80 border-slate-600/50 text-white hover:bg-slate-700/80" />
          </Carousel>
        </div>

        {/* Navigation dots for mobile */}
        <div className="flex justify-center gap-2 mt-8 lg:hidden">
          {softwareScreenshots.map((_, index) => (
            <div
              key={index}
              className="w-2 h-2 bg-slate-600 rounded-full opacity-50"
            />
          ))}
        </div>
